...
import { X, CheckCircle2, ShieldCheck, Lock, CreditCard, Smartphone, QrCode, Download, Sparkles } from 'lucide-react';
        ...
        {!isPaid ? (
          <div>
            <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 uppercase tracking-wider mb-2">
              <Lock className="w-3.5 h-3.5" /> Pembayaran Terenkripsi Personality.co
            </div>
            ...
            {/* Price Summary Box */}
            <div className="my-5 p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-slate-900 block">Dossier Lengkap 15 Halaman + Sertifikat</span>
              ...
              <div className="text-right">
                ...
                <span className="text-lg font-black text-indigo-600">Rp 49.000</span>
              </div>
            </div>
            ...
            <div className="space-y-2 mb-6">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-900">QRIS (Semua E-Wallet / M-Banking)</div>
                    <div className="text-[10px] text-slate-500">GoPay, OVO, Dana, ShopeePay, BCA, dll.</div>
                  </div>
                </div>
                {selectedMethod === 'qris' && <CheckCircle2 className="w-4 h-4 text-indigo-600" />}
              </button>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-900">Virtual Account Otomatis</div>
                    <div className="text-[10px] text-slate-500">BCA, Mandiri, BRI, BNI, Permata</div>
                  </div>
                </div>
                {selectedMethod === 'va' && <CheckCircle2 className="w-4 h-4 text-indigo-600" />}
              </button>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-900">Kartu Kredit / Debit Online</div>
                    <div className="text-[10px] text-slate-500">Visa, Mastercard, JCB, GPN</div>
                  </div>
                </div>
                {selectedMethod === 'card' && <CheckCircle2 className="w-4 h-4 text-indigo-600" />}
              </button>
            </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-2xl font-bold text-sm bg-slate-950 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 cursor-pointer transition-all disabled:opacity-50"
            >
              {isProcessing ? (
          ...
          <div className="text-center py-4">
            ...
            <div className="my-6 p-4 bg-slate-50 rounded-2xl border border-slate-200/80 text-left">
              <div className="flex items-center justify-between text-xs mb-2">
                ...
                <span className="font-mono font-bold text-slate-900">{licenseId || 'WWIQ-ID-992140'}</span>
              </div>
              <div className="flex items-center justify-between text-xs mb-2">
                <span className="text-slate-500">Skor Terverifikasi:</span>
                <span className="font-bold text-indigo-600">IQ {score} (Superior)</span>
              </div>
            ...
            <button
              onClick={() => alert(`Mengunduh Sertifikat Personality_IQ_${user.name.replace(/\s+/g, '_')}.pdf...`)}
              className="w-full py-4 rounded-2xl font-bold text-sm bg-slate-950 hover:bg-slate-900 text-white shadow-md flex items-center justify-center gap-2 cursor-pointer transition-all"
            >
